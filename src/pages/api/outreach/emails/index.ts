...
import { addEmail, listEmails } from '@server/features/outreach/service';
...
export const GET: APIRoute = (ctx) =>
  withUser(ctx, async (u) => {
    ...
    return json({ emails: await listEmails(u.id, campaignId, { status: q.get('status') ?? undefined, from: q.get('from') ?? undefined, to: q.get('to') ?? undefined }) });
  });
...
export const POST: APIRoute = (ctx) =>
  withPro(ctx, async (u) => {
    ...
    try {
      return json({ ok: true, ...(await addEmail(u.id, {
